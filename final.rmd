...
It is pretty well known that the winning teams of the La Liga tournament are typically one of Barcelona and Real Madrid. We decided to find the probabilities that each of these teams would in, given a past season.
...
Lastly, we wanted to look at the number of saves and goals of the champion of the LaLiga over the years. We want to see if we can make a linear regression model to predict the overall saves and goals of the winner. 
...
Looking at our results, There is a `r championNames$pr[1]*100`% chance that the winning team will be Barcelona, `r championNames$pr[2]*100`% chance that it will be Real Madrid, and `r (sum(championNames$pr)-(championNames$pr[1] + championNames$pr[2]))*100`% chance it will be one of the other teams. Next we will highlight the likely attributes of the winner of the 2020-2021 La Liga season. We can say with 95% confidence that the total number of points the winning team will have will fall between `r CI_pt[1]` to `r CI_pt[2]` and their total number of wins will be within `r round(CI_wins[1])` to `r round(CI_wins[2])`. Lastly we can make statistical predictions on the number of saves and goals that the winning team will have. Through our linear regression model we discovered with 95% confidence, the winning team will score between 80 and 97 goals and save between 95 and 116 shots.
...
<!-- ## Extra Graphs -- Not sure where/how to use -->

<!-- > Question: How does the winning teams saves and goals compare to non-winning teams? -->

<!-- We now want to start looking at both champion teams and non-champion teams. To do this we will perform another Linear regression on saves vs. goals for champions and non champions in each season. -->
...
For each champion team of each season, they have a top scorer that scores more goals than other players in the team and this consists of a sizable proportion of total goals of the team. 
```{R}
...
- expect: a high top scorer for champion teams
- do 2 sample proportion t tests.
```{R}
# lm_s = lm()
...
From the observation of the graph, we assume that the proportion of top scorer's goals in champions teams tend to be higher than those of non-champions teams. To verify our guess, we perform the hypothesis test where the null hypothesis is that the proportions are equal.  After doing the the t-test, we found that the p-value is 0.01166 and it is statistically significant so that we can reject the null hypothesis. Therefore, The proportion of top scorer's goals of all goals in champions teams tend to be higher than those of non-champions teams.